    ...
    <header className="bg-notion-surface border-b border-notion-border px-6 py-3 flex items-center gap-4">
      ...
      <div className="flex-1 max-w-sm ml-4">
        <div className="relative">
          <svg
            className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-notion-muted"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
              ...
              >
                ...
                <div className="max-h-80 overflow-y-auto">
                  {notifications.length === 0 ? (
                    <p className="text-center text-notion-muted text-sm py-8">
                      No notifications
                    </p>
                  ) : (
                    notifications.map((n) => (
                      <div
                        key={n._id}
                        className={`px-4 py-3 border-b border-notion-border last:border-0 hover:bg-notion-hover cursor-pointer transition-colors ${
                          !n.isRead ? "bg-notion-accent/5" : ""
                        }`}
                      >
                        <p className="text-xs text-notion-text leading-relaxed">
                          {n.message}
                        </p>
                        <p className="text-[10px] text-notion-muted mt-1">
                          {timeAgo(n.createdAt)}
                        </p>
                      </div>
                    ))
